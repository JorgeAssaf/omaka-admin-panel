...
const CardPedidos = ({
  ...
  const { idPedido, direccionPedido, nombreCliente, status } = data

  const [isSelect, setIsSelect] = useState(false)
  const colorStatus = variant?.includes('positive')
    ...
    <div className={variant?.includes('newRate') ? 'cardContainer row' : ''}>
      ...
      >
        <div>
          ...
          <ContentText>
            <SvgIconStyled component={Place} fontSize='small' />
            {stringCutting(direccionPedido, 30)}
          </ContentText>
          <ContentText>
            <SvgIconStyled component={Person} fontSize='small' />
